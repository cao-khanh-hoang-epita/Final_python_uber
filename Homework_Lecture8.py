...
def edit_job(job_id):
    ...
    if request.method == 'POST':
        ...
        collection.update_one({'_id': ObjectId(job_id)}, {'$set': {'name': name, 'description': description, 'wage': wage}})
        flash('Job edited successfully', 'success')
        return redirect(url_for('driver_map'))
...
@app.route('/add_to_cart/<job_id>', methods=['POST'])
@login_required
def add_to_cart(job_id):
    job = collection.find_one({'_id': ObjectId(job_id)})
    existing_item = cart.find_one({"name": job["name"], "description": job["description"], "wage": job["wage"], "user_id": current_user.id})

    if not existing_item:
        # If the item doesn't exist in the cart, add it.
        cart_item = {"name": job["name"], "description": job["description"], "wage": job["wage"], "user_id": current_user.id}
...
@login_required
def display_cart():
    cart_jobs = cart.find({"user_id": current_user.id})
    cart_jobs = [item for item in cart_jobs]
    total_price = sum(item["wage"] for item in cart_jobs)
    total_price = round(total_price, 2)
    return render_template("cart.html", cart_jobs=cart_jobs, total_price=total_price)
...
def checkout():
    if request.method == 'POST':
        cart_jobs = list(cart.find({"user_id": current_user.id}))
        cart.delete_many({"user_id": current_user.id})
        flash('Payment successful. Your order has been placed.', 'success')
        return redirect(url_for('profile'))
    cart_jobs = list(cart.find({"user_id": current_user.id}))
    total_price = sum(item["price"] for item in cart_jobs)
    total_price = round(total_price, 2)
    return render_template('checkout.html', cart_jobs=cart_jobs, total_price=total_price)
